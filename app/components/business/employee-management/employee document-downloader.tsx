...
interface EmployeeDocumentDownloaderProps {
  isOpen: boolean;
...
const EmployeeDocumentDownloader: React.FC<EmployeeDocumentDownloaderProps> = ({ 
  isOpen, 
  onOpenChange, 
  documents,
  employeeName
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Create a root folder with employee name
      const safeEmployeeName = employeeName.replace(/[^a-z0-9]/gi, '_');
      const rootFolder = `${safeEmployeeName}_Documents`;
      ...
      for (const doc of selectedDocuments) {
        ...
        try {
          ...
          const folderPath = `${rootFolder}/${doc.docType}/`;
      ...
      saveAs(zipBlob, `${safeEmployeeName}_Documents.zip`);
      ...
      setError(error instanceof Error ? error.message : "There was an error creating the download. Please try again.");
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh]">
        <DialogHeader>
          <DialogTitle>Download Employee Documents</DialogTitle>
          <DialogDescription>
...
export default EmployeeDocumentDownloader;